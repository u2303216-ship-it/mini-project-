    ...
    with open(CSV_PATH, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if not name:
                continue
            def g(col):
                try: return float(str(row.get(col,"0") or "0").strip())
                except: return 0.0
            foods.append({
                "name":     name,
...
def pick_best_fit(foods, target_kcal, exclude_names, tolerance=0.55):
    ...
    candidates = [f for f in foods if lo <= f["calories"] <= hi and f["name"] not in exclude_names]
    if not candidates:
...
def generate_weekly_plan(user):
    ...
    for day in DAYS:
        ...
        db.session.add(MealPlan(
            ...
            breakfast_protein=bf["protein"], breakfast_carbs=bf["carbs"], breakfast_fat=bf["fat"],
            lunch_name=lu["name"], lunch_calories=lu["calories"],
            lunch_protein=lu["protein"], lunch_carbs=lu["carbs"], lunch_fat=lu["fat"],
            dinner_name=di["name"], dinner_calories=di["calories"],
            dinner_protein=di["protein"], dinner_carbs=di["carbs"], dinner_fat=di["fat"],
...
class CalorieCalculator:
    ACTIVITY_MULTIPLIERS = {
        ...
        "Moderately Active": 1.55, "Active": 1.725,
    }
    GOAL_ADJUSTMENT = {"Fat Loss": -400, "Maintain Weight": 0, "Muscle Gain": 400}
...
def create_app():
    ...
    def register():
        d = request.json or {}
        if not all(k in d for k in ["full_name", "email", "password", "confirm_password"]):
    ...
    def login():
        ...
        if a and a.check_password(password):
            ...
            login_user(a)
            return jsonify({
                "message": "Login successful", "is_admin": True,
                "redirect": "/admin",
                "user_id": a.admin_id, "name": a.full_name, "email": a.email,
            })
        u = User.query.filter_by(email=email).first()
        if u and u.check_password(password):
            if u.status == "restricted":
                return jsonify({"message": "Your account has been restricted. Please contact the administrator."}), 403
    ...
    def calculate_calories():
        ...
        u = current_user
        u.age = age; u.gender = gender; u.height = height; u.weight = weight
        u.activity_level = activity; u.body_goal = goal
    ...
    @login_required
    def get_plan():
        plans = MealPlan.query.filter_by(user_id=current_user.user_id)\
        ...
        return jsonify([{
            ...
            "breakfast": {"name": p.breakfast_name, "calories": p.breakfast_calories,
                          "protein": p.breakfast_protein, "carbs": p.breakfast_carbs, "fat": p.breakfast_fat},
            "lunch":     {"name": p.lunch_name, "calories": p.lunch_calories,
                          "protein": p.lunch_protein, "carbs": p.lunch_carbs, "fat": p.lunch_fat},
            "dinner":    {"name": p.dinner_name, "calories": p.dinner_calories,
                          "protein": p.dinner_protein, "carbs": p.dinner_carbs, "fat": p.dinner_fat},
    ...
    def flag_inactive_users():
        ...
        stale = User.query.filter(
            User.status == "active",
            db.or_(User.last_login == None, User.last_login < cutoff)
        ).all()
        for u in stale:
            u.status = "inactive"
        if stale:
    ...
    def admin_get_user(uid):
        ...
        return jsonify({
            ...
            "age": u.age, "gender": u.gender, "height": u.height, "weight": u.weight,
            "activity_level": u.activity_level, "body_goal": u.body_goal,
            "maintenance_calories": u.maintenance_calories, "target_calories": u.target_calories,
        })
    ...
    def admin_get_users():
        ...
        return jsonify([{
            "user_id": u.user_id, "full_name": u.full_name, "email": u.email,
            "status": u.status or "active", "has_profile": u.age is not None,
            "last_login": u.last_login.strftime("%d %b %Y, %I:%M %p") if u.last_login else "Never",
            "body_goal": u.body_goal or "—", "target_calories": u.target_calories or 0,
        } for u in users])
    ...
    def admin_list_admins():
        ...
        return jsonify([{
            "user_id": a.admin_id, "full_name": a.full_name, "email": a.email,
    ...
    def admin_user_mealplan(uid):
        plans = MealPlan.query.filter_by(user_id=uid).order_by(MealPlan.plan_id).all()
        if not plans: return jsonify([])
        return jsonify([{
            "day": p.day_of_week,
            "breakfast": {"name": p.breakfast_name, "calories": p.breakfast_calories},
            "lunch":     {"name": p.lunch_name,     "calories": p.lunch_calories},
